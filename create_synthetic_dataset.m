function [features, labels, instrument_classes] = create_synthetic_dataset(n_per_class, fs, nfft, hop, n_mfcc)
    ...
    instrument_defs = {
        ...
        'Piano',    150,   1800,  16,  1.10,  0.010, 0.0008, 'piano';
        'Violin',   196,    880,   8,  1.05,  0.060, 0.0,    'bowed';
        'Trumpet',  200,    750,  15,  0.85,  0.018, 0.0,    'brass';
        'Guitar',    82,    330,   6,  2.10,  0.020, 0.00005,'plucked';
    };
    ...
    fprintf('Generating synthetic data (4 instruments):\n');
    ...
    for c = 1:n_classes
        ...
        for s = 1:n_per_class
            ...
            sig = sig + 0.005 * randn(1, n_samples);
    ...
    switch env_type
        case 'plucked'
            ...
            for n = 1:n_notes
                ...
                decay_rate = 6.0 + rand() * 4.0;   % fast: 6.5-10 s^-1
                ...
                % Small pick transient — kept low to avoid SVM confusing
                % high-freq burst with brass
                pick_len = round(0.008 * fs);
                p_end    = min(onset + pick_len - 1, n_samples);
                pick_noise = 0.04 * randn(1, pick_len) .* linspace(1,0,pick_len);
                sig(onset:p_end) = sig(onset:p_end) + pick_noise(1:p_end-onset+1);
                sig = sig + note .* env + noise_lvl * randn(1,n_samples) .* env;
            end

        case {'bowed', 'brass'}
            % Violin / Trumpet  ('blown'/Flute removed)
            n_notes   = 2 + randi(2);
        ...
        case 'piano'
            ...
            for n = 1:n_notes
                f0  = exp(log(f0_lo) + rand() * log(f0_hi/f0_lo));
                n_h = min(n_harm, floor(f_nyq/f0));
                if n_h < 1, n_h = 1; end
                onset       = round(note_times(n) * fs) + 1;
                attack      = round(0.040 * fs);
                decay1      = round(0.25 * fs);
                sus_level   = 0.55 + rand() * 0.20;
                decay2_rate = 0.30 + rand() * 0.20;
                note = zeros(1, n_samples);
                for h = 1:n_h
                    f_h = h * f0 * sqrt(1 + 0.0005 * h^2);
                    if f_h >= f_nyq, break; end
                note = note + (1/h^rolloff) * sin(2*pi*f_h*t + 2*pi*rand());
                end
                env = zeros(1, n_samples);
...
function sig_out = add_reverb(sig, fs, env_type)
    ...
    switch env_type
        case 'plucked'
            rt60 = 0.04 + rand() * 0.06;   % 0.04-0.10s: body resonance but dry
            wet  = 0.08 + rand() * 0.08;
        case {'bowed', 'brass'}
            if strcmp(env_type, 'brass')
                rt60 = 0.15 + rand() * 0.2;
                wet  = 0.2  + rand() * 0.15;
            else
                rt60 = 0.3  + rand() * 0.3;
