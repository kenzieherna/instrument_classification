...
fprintf('=== DIAGNOSTIC: Why Classification May Fail ===\n\n');
...
try
    ...
    for c = 1:length(classes)
        fprintf(' | %8s', classes{c}(1:min(8,length(classes{c}))));
    end
    fprintf('\n');
    fprintf('%s\n', repmat('-', 1, 75));
    ...
    % Analysis
    pred_counts = accumarray(pred_test, 1, [length(classes),1]);
    [~, max_idx] = max(pred_counts);
    ...
    if max_pct > 40
        ...
        fprintf('Features for %s may overlap other classes.\n\n', classes{max_idx});
    end
    ...
    for feat_idx = 1:min(5, size(features,2))
        means = zeros(1, length(classes));
        stds  = zeros(1, length(classes));
        for c = 1:length(classes)
            idx = labels == c;
    ...
    for c = 1:length(classes)
        ...
        y   = features(idx,1);
        scatter(x, y, 40, 'DisplayName', classes{c}); hold on;
    end
    set(gca,'XTick',1:length(classes),'XTickLabel',classes);
    xtickangle(45); ylabel('Feature 1 Value');
    title('Feature 1: Spectral Centroid'); grid on;
    ...
    if size(features,2) >= 2
        ...
        set(gca,'XTick',1:length(classes),'XTickLabel',classes);
        xtickangle(45); ylabel('Feature 2 Value');
        title('Feature 2: Spectral Rolloff'); grid on;
    end
    ...
    if size(features,2) >= 2
        ...
        for c = 1:length(classes)
            ...
            scatter(features(idx,1), features(idx,2), 40, ...
                'DisplayName', classes{c}); hold on;
        end
        xlabel('Feature 1'); ylabel('Feature 2');
        title('2D Feature Space'); legend('FontSize',8); grid on;
    end
...
%% ========================================================================
%% SYNTHETIC DATA GENERATION (4 instruments — Flute removed)
%% ========================================================================
...
function feat = extract_basic_features(sig, fs)
    ...
    energy_env       = abs(sig);
    energy_centroid  = sum(t_vec .* energy_env) / (sum(energy_env) + 1e-10);
